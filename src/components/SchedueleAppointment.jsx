...
const SchedueleAppointment = ({ profile, doctorAppointments, getDoctorAppointments }) => {
    ...
    const findAllowedUser = () => {
        const allowedUser = user.allowedUsers.find((allowed) => allowed === profile._id)
    ...
    return (
        <div style={{ background: "#ddf4f5" }} className="my-5">
            <ModalAllowUser show={show} handleClose={() => setShow(false)} profileId={profile._id} />
            {!user ? (
                            ...
                            <Toaster
                                text={
                                    errors.length > 0 ? (
                                        ...
                                        <p className="text-left mb-1">Appointment was successfuly booked</p>
                                    )
                                }
                                ...
                                <Col md="7">
                                    ...
                                    >
                                        ...
                                        {["Check up", "Follow up"].map((reason, index) => (
                                            <option key={`${reason}andReason${index}`} value={`${reason}`}>
                                                {reason}
                                            </option>
                                        ))}
                                    </Form.Control>
                            ...
                            <Row className="mt-5 mx-1">
                                {hours.length === 0 && (
                                    <Col xs="12">
                                        <h5 className="text-center my-5" style={{ color: "#ff804a" }}>
                                            <i className="far fa-calendar-times"></i> There are no appointments for this
                                            day
                                        </h5>
                                    </Col>
                                )}
                                {hours.map((h, index) => (
                                    <Col xs={4} sm={3} key={index} className="mb-4 getBiger">
                                        <p
                                            className={`${h === hour ? "hoursOfDaySelected" : "hoursOfDay"} mb-0`}
                                            onClick={() => setHour(h)}
                                        >
                                            {h}
                                        </p>
